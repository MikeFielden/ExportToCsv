...
valArr.push(options.rowDelim);return valArr.join(options.colDelim);});return csv.concat(output);};var exportToCsv=function(selector,userOpts){var defaultOptions={colDelim:',',rowDelim:'\r\n',headerRow:true,fileName:'data.csv',autoDownload:true};var csv=[];var a=document.createElement('a');var $table,$theads,$bodyRows;var blob,url;options=helpers.defaults(userOpts||{},defaultOptions);if(Array.isArray.call(this,selector)&&selector.length>0){if(options.headerRow){buildArrayHeaderRow(selector,csv);}
