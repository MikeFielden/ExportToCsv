javascript: (function () {
	...
	return result;},isString:function(value){return typeof value=='string'||value&&typeof value=='object'&&toString.call(value)=='[object String]'||false;},defaults:function(obj){Array.prototype.forEach.call(Array.prototype.slice.call(arguments,1),function(source){if(source){for(var prop in source){if(obj[prop]===void 0)obj[prop]=source[prop];}}});return obj;}};var buildDOMHeaderRow=function($theads,csv){var headers=[].map.call($theads,function(el){return el.innerHTML.replace(/<(?:.|\n)*?>/gm,'');});headers.push(options.rowDelim);csv.push(headers.join(options.colDelim));};var buildDOMBody=function($trs,csv){var rows=[].map.call($trs,function($tr){var thing=[].map.call($tr.children,function($td){return'"'+$td.innerHTML.replace(/<(?:.|\n)*?>/gm,'')+'"';});thing.push(options.rowDelim);return thing.join(options.colDelim);});return csv.concat(rows);};var buildArrayHeaderRow=function(input,csv){var header;if(typeof input[0]!=='object'){header=[];}else{header=Object.keys(input[0]);}
